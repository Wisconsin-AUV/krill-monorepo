...
export function DropdownItem(props: { to?: string } & HeadlessMenuItemProps<'button'>) {
  return (
    <HeadlessMenuItem
      as={props.to ? NavLink : 'button'}
...
export function DropdownHeading({ className, ...props }: HeadlessMenuHeadingProps) {
  return (
    <HeadlessMenuHeading
      ...
      className={clsx(
        className,
        'col-span-full grid grid-cols-[1fr,auto] gap-x-12 px-3.5 pt-2 pb-1 text-sm/5 font-medium text-zinc-500 sm:px-3 sm:text-xs/5 dark:text-zinc-400',
      )}
    />
  )
}
