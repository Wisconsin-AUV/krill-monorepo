...
export function Timeline({
  ...
  return (
    <div className="border-t border-white/10 bg-zinc-900 px-4 pt-2 pb-3 select-none">
      <div className="mb-1.5 flex justify-between text-xs/5 text-zinc-400 tabular-nums">
      ...
      >
        <input
          ...
          className="sr-only"
        />
        <div className="absolute inset-y-2 left-0 flex w-full overflow-hidden rounded bg-zinc-800">
          {frameClass &&
            frames.map((f, i) => (
        ...
        {hover !== null && hover !== index && (
          ...
          >
            <span className="absolute -top-6 -translate-x-1/2 rounded bg-zinc-700 px-1.5 py-0.5 text-[11px] whitespace-nowrap text-white tabular-nums">
              {frames[hover].index}
            </span>
          </div>
        )}
        <div
          className="pointer-events-none absolute inset-y-0 w-0.5 -translate-x-1/2 rounded-full bg-sky-400 shadow-[0_0_0_2px] shadow-zinc-900"
